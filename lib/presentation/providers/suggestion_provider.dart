...
class SuggestionNotifier extends StateNotifier<SuggestionState> {
  ...
  Future<void> loadSuggestions() async {
    ...
    try {
      ...
      final posts = data.map((e) => PostModel(
        id: e['id'] as String,
        userId: e['user_id'] as String,
        content: e['content'] as String? ?? '',
        mediaUrls: (e['media_urls'] as List<dynamic>?)?.cast<String>() ?? [],
        mediaType: MediaType.text,
        latitude: (e['latitude'] as num?)?.toDouble() ?? 0,
        longitude: (e['longitude'] as num?)?.toDouble() ?? 0,
        contextTag: e['context_tag'] as String?,
        reactionCounts: e['reaction_counts'] != null ? Map<String, int>.from(e['reaction_counts'] as Map) : {},
        createdAt: e['created_at'] != null ? DateTime.parse(e['created_at'] as String) : null,
        userUsername: e['username'] as String?,
        userDisplayName: e['display_name'] as String?,
        userAvatarUrl: e['avatar_url'] as String?,
        distanceMeters: (e['distance_meters'] as num?)?.toDouble() ?? 0,
