...
class PageNotifier extends StateNotifier<PageState> {
  ...
  Future<void> loadNearbyPages({String? category}) async {
    ...
    try {
      ...
      if (data.isNotEmpty) dev.log('First page: ${data.first}', name: 'PageProvider');
      ...
      state = state.copyWith(pages: items, isLoading: false);
    } catch (e, st) {
      dev.log('loadNearbyPages error: $e', name: 'PageProvider', error: e, stackTrace: st);
  ...
  Future<void> getPagePosts(String pageId) async {
    ...
    try {
      ...
      state = state.copyWith(pagePosts: posts, isLoading: false);
    } catch (e) {
      state = state.copyWith(isLoading: false, error: e.toString());
    }
  }
