-- =============================================================================
-- MIGRATION 039 (FINAL — merged from 3 conflicting 039_* files)
-- =============================================================================
-- Sources merged:
--   • 039_comprehensive_security_fixes.sql
--   • 039_security_fixes_final.sql
--   • 039_security_hardening.sql  ← primary source (most complete)
--
-- Vulnerabilities fixed:
--   [CRITICAL-1] Hardcoded service_role JWT in migration 036
...
--   [MED-8]      conversations INSERT RLS allows non-participant creation
--   [MED-9]      send_message — no rate limiting at DB level
--   [MED-10]     media_url accepts any arbitrary URL (SSRF risk)
--   [REVOKE]     anon/authenticated EXECUTE grants on sensitive functions
-- =============================================================================


-- ─────────────────────────────────────────────────────────────
-- PART A: REVOKE anon/authenticated from sensitive functions
-- (merged from 039_comprehensive_security_fixes + 039_security_fixes_final)
-- ─────────────────────────────────────────────────────────────
...
-- Revoke admin-only functions from authenticated → service_role only
...
-- Re-grant to roles that legitimately need access
...
-- ─────────────────────────────────────────────────────────────
-- PART B: Fix mutable search_path on 7 vulnerable functions
-- ─────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF v_edge_url IS NULL OR v_edge_url = 'https:///functions/v1/send-push' THEN
    RAISE WARNING 'push_trigger: app.supabase_url not set.';
    RETURN NEW;
  END IF;
...
-- ─────────────────────────────────────────────────────────────
-- PART J: Constrain privacy_settings JSONB keys (MED-7)
-- ─────────────────────────────────────────────────────────────
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE constraint_name = 'chk_privacy_settings_keys' AND table_name = 'profiles'
  ) THEN
    ALTER TABLE public.profiles
    ADD CONSTRAINT chk_privacy_settings_keys CHECK (
      privacy_settings IS NULL OR (
        (privacy_settings - ARRAY[
          'show_location','show_online_status','show_last_seen',
          'show_followers','show_following',
          'allow_messages_from_strangers','invisible_mode','private_account'
        ]) = '{}'
        AND NOT EXISTS (
          SELECT 1 FROM jsonb_each(privacy_settings) kv
...
-- ─────────────────────────────────────────────────────────────
-- PART L: Enable RLS on spatial_ref_sys (PostGIS)
-- ─────────────────────────────────────────────────────────────
...
CREATE POLICY "Allow service_role full access spatial_ref_sys"
  ON public.spatial_ref_sys FOR ALL TO service_role USING (true) WITH CHECK (true);


-- =============================================================================
-- POST-DEPLOYMENT MANUAL STEPS (required after running this migration)
-- =============================================================================
-- 1. Supabase Dashboard → Settings → API → Regenerate service_role key
-- 2. Dashboard → Settings → Vault:
--      SERVICE_ROLE_KEY          = <new service_role JWT>
--      INTERNAL_FUNCTION_SECRET  = <new 64-char random string>
-- 3. Dashboard → Edge Functions → send-push → Secrets:
--      INTERNAL_FUNCTION_SECRET  = same as Vault above
-- 4. Run in SQL Editor:
